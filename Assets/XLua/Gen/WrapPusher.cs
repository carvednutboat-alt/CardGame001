...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        public void PushCardTargetType(RealStatePtr L, CardTargetType val)
        {
            if (CardTargetType_TypeID == -1)
            {
			    bool is_first;
                CardTargetType_TypeID = getTypeId(L, typeof(CardTargetType), out is_first);
				
				if (CardTargetType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(CardTargetType));
				    CardTargetType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, CardTargetType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, CardTargetType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for CardTargetType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, CardTargetType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out CardTargetType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != CardTargetType_TypeID)
				{
				    throw new Exception("invalid userdata for CardTargetType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for CardTargetType");
                }
				val = (CardTargetType)e;
                
            }
            else
            {
                val = (CardTargetType)objectCasters.GetCaster(typeof(CardTargetType))(L, index, null);
            }
        }
		
        public void UpdateCardTargetType(RealStatePtr L, int index, CardTargetType val)
        {
